...
func (s *Store) ListForReview(ctx context.Context) ([]AdminItem,error) {
	rows,err:=s.pool.Query(ctx,`
		select o.id::text,o.legal_name,o.cnpj,o.status::text,p.title,u.name,o.submitted_at,
	...
	for rows.Next(){
		var item AdminItem
		if err:=rows.Scan(&item.ID,&item.LegalName,&item.CNPJ,&item.Status,&item.ProposalTitle,&item.CommercialName,&item.SubmittedAt,&item.ContractID,&item.ContractStatus,&item.UpdatedAt);err!=nil{return nil,err}
...
func (s *Store) AdminByID(ctx context.Context,id string)(AdminDetail,error){
	...
	err:=s.pool.QueryRow(ctx,`
		select o.proposal_acceptance_id::text,p.title,u.name,coalesce(o.review_notes,''),o.submitted_at,o.reviewed_at,
		       coalesce(c.id::text,''),coalesce(c.status::text,'')
		from onboardings o
		join proposal_acceptances pa on pa.id=o.proposal_acceptance_id
		join proposals p on p.id=pa.proposal_id
		join users u on u.id=p.created_by
		left join contracts c on c.onboarding_id=o.id and c.status<>'cancelled'
		where o.id=$1
	`,id).Scan(&acceptanceID,&detail.ProposalTitle,&detail.CommercialName,&detail.ReviewNotes,&detail.SubmittedAt,&detail.ReviewedAt,&detail.ContractID,&detail.ContractStatus)
	if err!=nil{return AdminDetail{},err}
	...
	for rows.Next(){
		...
		if err:=rows.Scan(&document.ID,&document.DocumentType,&document.StorageKey,&document.OriginalFilename,&document.MIMEType,&document.SizeBytes,&document.SHA256);err!=nil{return AdminDetail{},err}
...
func (s *Store) Review(ctx context.Context,id,userID,status,notes string) error {
	...
	`,id).Scan(&current,&hasContract);err!=nil{return err}

	switch status {
	case "under_review":
		if current!="submitted"&&current!="under_review"{return fmt.Errorf("onboarding cannot enter review from %s",current)}
	case "correction_requested":
		if hasContract{return fmt.Errorf("cannot request correction after contract generation")}
		if current!="submitted"&&current!="under_review"{return fmt.Errorf("onboarding cannot request correction from %s",current)}
	case "approved":
		if current!="submitted"&&current!="under_review"&&current!="approved"{return fmt.Errorf("onboarding cannot be approved from %s",current)}
	}
