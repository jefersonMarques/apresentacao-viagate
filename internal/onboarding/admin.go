...
func (s *Store) ListForReview(ctx context.Context) ([]AdminItem, error) {
	...
	for rows.Next() {
		var item AdminItem
		if err := rows.Scan(&item.ID, &item.LegalName, &item.CNPJ, &item.Status, &item.ProposalTitle, &item.CommercialName, &item.SubmittedAt, &item.ContractID, &item.ContractStatus, &item.UpdatedAt); err != nil {
			return nil, err
		}
