...
func (s *Store) ListTemplates(ctx context.Context) ([]domain.ContractTemplate, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Name, &item.Description, &item.CurrentVersion, &item.IsActive); err != nil {
			return nil, err
		}
...
func (s *Store) SetDefaultTemplate(ctx context.Context, templateID string) error {
	...
	if _, err := tx.Exec(ctx, `update contract_templates set is_default=false where is_default=true`); err != nil {
		return err
	}
	command, err := tx.Exec(ctx, `update contract_templates set is_default=true where id=$1 and is_active=true`, templateID)
	if err != nil {
		return err
	}
...
func (s *Store) LatestTemplateVersion(ctx context.Context, templateID string) (domain.ContractTemplateVersion, error) {
	...
	`, templateID).Scan(&version.ID, &version.ContractTemplateID, &version.VersionNumber, &version.Markdown, &version.TemplateHash)
	return version, err
}
...
func (s *Store) CreateGeneratedContract(ctx context.Context, onboardingID, proposalVersionID, templateVersionID, markdown, html, storageKey string, documentHash []byte, createdBy string) (domain.Contract, error) {
	...
	`, onboardingID, proposalVersionID, templateVersionID, markdown, html, storageKey, documentHash, createdBy).Scan(
		&contract.ID,
		&contract.OnboardingID,
		&contract.ProposalVersionID,
		&contract.TemplateVersionID,
		&contract.Status,
		&contract.RenderedMarkdown,
...
func (s *Store) AddSigner(ctx context.Context, contractID, signerType, name, email, cpf, role string, signOrder int) (domain.ContractSigner, error) {
	...
	`, contractID, signerType, name, email, cpf, role, signOrder).Scan(
		&signer.ID,
		&signer.ContractID,
		&signer.SignerType,
		&signer.Name,
...
func (s *Store) SignerByPublicToken(ctx context.Context, token string) (SignerAccess, error) {
	...
	`, token).Scan(
		...
		&access.Signer.SignerType,
		&access.Signer.Name,
		&access.Signer.Email,
		&access.Signer.CPF,
		&access.Signer.Role,
		&access.Signer.SignOrder,
		&access.Signer.Status,
		&access.Signer.SignedAt,
		&access.Contract.ID,
		&access.Contract.OnboardingID,
		&access.Contract.ProposalVersionID,
		&access.Contract.TemplateVersionID,
		&access.Contract.Status,
		&access.Contract.RenderedMarkdown,
		&access.Contract.RenderedHTML,
		&access.Contract.PDFStorageKey,
		&access.Contract.DocumentSHA256,
		&access.Contract.EvidenceReportStorageKey,
		&access.Contract.EvidenceReportSHA256,
		&access.Contract.FinalPackageStorageKey,
		&access.Contract.FinalPackageSHA256,
		&access.Contract.GeneratedAt,
		&access.Contract.SentAt,
		&access.Contract.FullySignedAt,
		&access.Contract.FinalizedAt,
	)
...
func (s *Store) VerifyChallenge(ctx context.Context, signerID string, otpHash []byte, sessionID string, ip net.IP, userAgent string) error {
	tx, err := s.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return err
	}
	...
	var attempts int
	err = tx.QueryRow(ctx, `
		select ch.id::text,ch.otp_hash,ch.attempts,s.contract_id::text,c.document_sha256
	...
	if attempts >= 5 {
		return fmt.Errorf("maximum OTP attempts reached")
	}
	...
	if _, err := tx.Exec(ctx, `update contract_signers set status='verified' where id=$1`, signerID); err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("record OTP identity verification: %w", err)
	}
	if command.RowsAffected() != 1 {
		return fmt.Errorf("pending OTP identity verification not found")
	}
	...
	`, contractID, signerID, documentHash, nullableIP(ip), userAgent, sessionID); err != nil {
		return fmt.Errorf("record OTP verification event: %w", err)
	}
...
func (s *Store) Sign(ctx context.Context, signerID string, documentHash []byte, sessionID string, ip net.IP, userAgent string) (string, bool, error) {
	...
	if status != "verified" {
		return "", false, fmt.Errorf("signer identity is not verified")
	}
	if !bytes.Equal(storedHash, documentHash) {
	...
	if _, err := tx.Exec(ctx, `
		...
		where id=$1
	`, signerID, documentHash, sessionID, legaltext.SignatureConsentVersion, legaltext.SignatureConsentText, consentHash); err != nil {
		return "", false, err
	}
	if _, err := tx.Exec(ctx, `
		insert into signature_events(contract_id,contract_signer_id,event_type,document_hash,ip_address,user_agent,session_id,metadata)
	...
	`, contractID, signerID, documentHash, nullableIP(ip), userAgent, sessionID, legaltext.SignatureConsentVersion, fmt.Sprintf("%x", consentHash)); err != nil {
