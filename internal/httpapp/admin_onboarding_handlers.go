...
func (a *App) adminOnboardings(w http.ResponseWriter, r *http.Request) {
	...
	allowAll := access.Can(user, access.CustomerResponsesReadAll) || access.Can(user, access.OnboardingReview)
	if !allowAll && !access.Can(user, access.CustomerResponsesReadOwn) {
		http.Error(w, "acesso negado", http.StatusForbidden)
		return
	}
	items, err := a.onboardingStore.ListVisible(r.Context(), user.ID, allowAll)
	if err != nil {
		a.logger.Error("list onboardings failed", "error", err)
		http.Error(w, "não foi possível carregar os cadastros", http.StatusInternalServerError)
		return
	}
...
func (a *App) adminOnboardingDetail(w http.ResponseWriter, r *http.Request) {
	...
	allowAll := access.Can(user, access.CustomerResponsesReadAll) || access.Can(user, access.OnboardingReview)
	if !allowAll && (!access.Can(user, access.CustomerResponsesReadOwn) || detail.OwnerUserID != user.ID) {
		http.Error(w, "acesso negado", http.StatusForbidden)
		return
	}
	message := ""
	...
	case "contract":
		message = "Contrato disponível e notificação de assinatura garantida na fila do Brevo."
	case "correction":
		message = "Correção solicitada e cliente notificado."
	}
...
func (a *App) reviewOnboarding(w http.ResponseWriter, r *http.Request) {
	...
	if status == "correction_requested" && notes == "" {
		http.Redirect(w, r, "/admin/onboardings/"+id+"?error="+queryEscape("Informe o que precisa ser corrigido."), http.StatusSeeOther)
		return
	}
	if err := a.onboardingStore.Review(r.Context(), id, user.ID, status, notes); err != nil {
		...
		http.Redirect(w, r, "/admin/onboardings/"+id+"?error="+queryEscape("Não foi possível alterar o status: "+err.Error()), http.StatusSeeOther)
		return
	}
	...
	if status == "correction_requested" {
		if err := a.sendOnboardingCorrection(r, id, detail.Onboarding.ProposalAcceptanceID, detail.Onboarding.CompanyResponsibleName, detail.Onboarding.CompanyResponsibleEmail, notes); err != nil {
			...
			http.Redirect(w, r, "/admin/onboardings/"+id+"?error="+queryEscape("Correção registrada, mas o e-mail não entrou na fila."), http.StatusSeeOther)
			return
		}
		http.Redirect(w, r, "/admin/onboardings/"+id+"?result=correction", http.StatusSeeOther)
		return
	}
	if status == "approved" {
		if _, _, err := a.ensureContractDelivery(r.Context(), id); err != nil {
			...
			http.Redirect(w, r, "/admin/onboardings/"+id+"?error="+queryEscape("Cadastro aprovado, mas a geração/envio do contrato falhou. Use Tentar novamente."), http.StatusSeeOther)
			return
		}
	}
...
func (a *App) retryOnboardingContract(w http.ResponseWriter, r *http.Request) {
	user, _ := currentUser(r.Context())
	id := chi.URLParam(r, "id")
	accessData, created, err := a.ensureContractDelivery(r.Context(), id)
	if err != nil {
		...
		http.Redirect(w, r, "/admin/onboardings/"+id+"?error="+queryEscape("Não foi possível gerar ou reenviar o contrato: "+err.Error()), http.StatusSeeOther)
		return
	}
	_, _ = a.pool.Exec(r.Context(), `
	...
	`, user.ID, id, accessData.ContractID, created)
...
func (a *App) adminOnboardingDocument(w http.ResponseWriter, r *http.Request) {
	user, _ := currentUser(r.Context())
	id := chi.URLParam(r, "id")
	if !access.Can(user, access.CustomerDocumentsRead) {
		http.Error(w, "acesso negado", http.StatusForbidden)
		return
	}
	detail, err := a.onboardingStore.AdminByID(r.Context(), id)
	...
	allowAll := access.Can(user, access.CustomerResponsesReadAll) || access.Can(user, access.OnboardingReview)
	if !allowAll && detail.OwnerUserID != user.ID {
		http.Error(w, "acesso negado", http.StatusForbidden)
		return
	}
	...
	if err != nil {
		a.logger.Error("presign onboarding document failed", "error", err)
		http.Error(w, "não foi possível disponibilizar o documento", http.StatusInternalServerError)
		return
	}
...
func (a *App) sendOnboardingCorrection(r *http.Request, onboardingID, acceptanceID, name, emailAddress, notes string) error {
	...
	if err := a.proposalStore.CreateCustomerResumeToken(r.Context(), acceptanceID, hash, expires); err != nil {
		return err
	}
