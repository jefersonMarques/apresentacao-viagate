...
func (a *App) login(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		render(r.Context(), w, http.StatusBadRequest, templates.Login("Dados inválidos."))
		return
	}
	...
	if err != nil || credentials.User.Status != "active" || !security.VerifyPassword(credentials.PasswordHash, password) {
		if recordErr := a.authStore.RecordLoginFailure(r.Context(), email, ip); recordErr != nil {
			a.logger.Error("record login failure failed", "error", recordErr)
		}
		time.Sleep(250 * time.Millisecond)
	...
	if err := a.authStore.ClearLoginFailures(r.Context(), email, ip); err != nil {
		a.logger.Warn("clear login failures failed", "error", err, "user_id", credentials.User.ID)
	}
...
func (a *App) invitationPage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			http.Error(w, "Convite inválido ou expirado.", http.StatusGone)
			return
		}
		http.Error(w, "Não foi possível carregar o convite.", http.StatusInternalServerError)
		return
	}
...
func (a *App) acceptInvitation(w http.ResponseWriter, r *http.Request) {
	...
	if password != r.FormValue("confirm_password") {
		render(r.Context(), w, http.StatusBadRequest, templates.InvitationPage(user.Name, user.Email, token, "As senhas são diferentes."))
		return
	}
	passwordHash, err := security.HashPassword(password)
	if err != nil {
		render(r.Context(), w, http.StatusBadRequest, templates.InvitationPage(user.Name, user.Email, token, err.Error()))
		return
	}
	...
	if err != nil {
		...
		render(r.Context(), w, http.StatusInternalServerError, templates.InvitationPage(user.Name, user.Email, token, "Não foi possível ativar o acesso."))
		return
	}
...
func (a *App) inviteUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Redirect(w, r, "/admin/users?error="+queryEscape("Informe um e-mail válido."), http.StatusSeeOther)
		return
	}
	...
	userID, err := a.authStore.CreateManagedInvitation(r.Context(), emailAddress, name, role, hash, time.Now().Add(a.cfg.Session.InviteTTL), actor.ID)
	if err != nil {
		a.logger.Error("create invitation failed", "error", err)
		http.Redirect(w, r, "/admin/users?error="+queryEscape("Não foi possível criar o convite: "+err.Error()), http.StatusSeeOther)
		return
	}
	...
	if err := notifications.Enqueue(r.Context(), a.pool, name, emailAddress, "Convite para a plataforma ViaGate", htmlBody, "Acesse "+link); err != nil {
		...
		http.Redirect(w, r, "/admin/users?error="+queryEscape("Convite criado, mas o e-mail não entrou na fila do Brevo."), http.StatusSeeOther)
		return
	}
	_, _ = a.pool.Exec(r.Context(), `insert into audit_events(actor_user_id,event_type,resource_type,resource_id,ip_address,user_agent,metadata) values($1,'user.invited','user',$2,$3,$4,jsonb_build_object('role',$5))`, actor.ID, userID, requestIP(r), r.UserAgent(), role)
