...
func (a *App) profilePage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "não foi possível carregar o perfil", http.StatusInternalServerError)
		return
	}
	message := ""
	if r.URL.Query().Get("saved") == "1" {
		message = "Perfil atualizado. Os novos dados serão usados nas próximas versões salvas."
	}
...
func (a *App) updateProfile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || name == "" {
		...
		render(r.Context(), w, http.StatusBadRequest, templates.ProfilePage(current, profile, "", "Informe nome e e-mail válidos."))
		return
	}
	...
	if err != nil {
		...
		profile.JobTitle = jobTitle
		render(r.Context(), w, http.StatusBadRequest, templates.ProfilePage(current, profile, "", "Telefone inválido."))
		return
	}
	photoURL, err := cleanCommercialImageURL(r.FormValue("photo_url"))
	if err != nil {
		a.renderProfileValidationError(w, r, current.ID, name, email, phone, jobTitle, "Foto inválida. Envie uma imagem novamente.")
		return
	}
	linkedInURL, err := cleanProfileURL(r.FormValue("linkedin_url"))
	if err != nil {
		a.renderProfileValidationError(w, r, current.ID, name, email, phone, jobTitle, "URL do LinkedIn inválida.")
		return
	}
	instagramURL, err := cleanProfileURL(r.FormValue("instagram_url"))
	if err != nil {
		a.renderProfileValidationError(w, r, current.ID, name, email, phone, jobTitle, "URL do Instagram inválida.")
		return
	}
	...
	if err := a.authStore.UpdateProfile(r.Context(), current.ID, name, email, phone, jobTitle, photoURL, linkedInURL, instagramURL); err != nil {
		...
		profile, _ := a.authStore.Profile(r.Context(), current.ID)
		render(r.Context(), w, http.StatusBadRequest, templates.ProfilePage(current, profile, "", "Não foi possível salvar o perfil. Verifique se o e-mail já está em uso."))
		return
	}
