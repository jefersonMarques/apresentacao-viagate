...
func (s *Store) SaveDraft(ctx context.Context,userID string,allowAll bool,input EditorInput)(SavedDraft,error){
	...
	if input.ProposalID==""{
		...
		if clientID==""{
			if err:=tx.QueryRow(ctx,`
				insert into clients(legal_name,trade_name,cnpj,email,phone,created_by)
	...
	contentJSON,err:=json.Marshal(input.Content);if err!=nil{return SavedDraft{},err}
	conditionsJSON,err:=json.Marshal(input.Conditions);if err!=nil{return SavedDraft{},err}
	...
	if err==pgx.ErrNoRows{
		err=tx.QueryRow(ctx,`select coalesce(max(version_number),0)+1 from proposal_versions where proposal_id=$1`,input.ProposalID).Scan(&draft.VersionNumber);if err!=nil{return SavedDraft{},err}
		err=tx.QueryRow(ctx,`
		...
		if err!=nil{return SavedDraft{},err}
		...
		`,draft.VersionID,input.PricingModel,contentJSON,conditionsJSON,input.MinimumInvoice,input.SetupFee,input.ContentHash);err!=nil{return SavedDraft{},err}
		if _,err:=tx.Exec(ctx,`delete from proposal_items where proposal_version_id=$1`,draft.VersionID);err!=nil{return SavedDraft{},err}
	}
	...
	for _,item:=range input.Items{
		...
		`,draft.VersionID,item.GroupName,item.Label,item.Unit,item.Price,item.IsOptional,item.SortOrder,metadata);err!=nil{return SavedDraft{},err}
	}
...
func (s *Store) EditorByID(ctx context.Context,userID,proposalID string,allowAll bool)(EditorInput,SavedDraft,error){
	...
	err:=s.pool.QueryRow(ctx,`
		select p.id::text,p.title,p.valid_until,p.created_by::text,c.legal_name,coalesce(c.trade_name,''),coalesce(c.cnpj,''),coalesce(c.email::text,''),coalesce(c.phone,'')
		from proposals p join clients c on c.id=p.client_id where p.id=$1
	`,proposalID).Scan(&input.ProposalID,&input.Title,&input.ValidUntil,&owner,&input.ClientLegalName,&input.ClientTradeName,&input.ClientCNPJ,&input.ClientEmail,&input.ClientPhone)
	if err!=nil{return EditorInput{},SavedDraft{},err};if !allowAll&&owner!=userID{return EditorInput{},SavedDraft{},fmt.Errorf("proposal access denied")}
