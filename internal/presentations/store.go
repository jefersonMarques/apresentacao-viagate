...
func (s *Store) SaveDraft(ctx context.Context,userID string,allowAll bool,input EditorInput)(SavedDraft,error){
	...
	if input.PresentationID==""{
		...
		if clientID=="" && input.ClientLegalName!=""{
			if err:=tx.QueryRow(ctx,`insert into clients(legal_name,trade_name,cnpj,created_by) values($1,nullif($2,''),nullif($3,''),$4) returning id::text`,input.ClientLegalName,input.ClientTradeName,input.ClientCNPJ,userID).Scan(&clientID);err!=nil{return SavedDraft{},err}
		}else if clientID!=""&&input.ClientLegalName!=""{
			if _,err:=tx.Exec(ctx,`update clients set legal_name=$2,trade_name=nullif($3,''),updated_at=now() where id=$1 and created_by=$4`,clientID,input.ClientLegalName,input.ClientTradeName,userID);err!=nil{return SavedDraft{},err}
		}
		if err:=tx.QueryRow(ctx,`insert into presentations(client_id,title,status,created_by) values(nullif($1,'')::uuid,$2,'draft',$3) returning id::text`,clientID,input.Title,userID).Scan(&input.PresentationID);err!=nil{return SavedDraft{},err}
	}else{
	...
	content.Client.LegalName=input.ClientLegalName;content.Client.TradeName=input.ClientTradeName;content.Client.CNPJ=input.ClientCNPJ;content.Client.LogoURL=input.ClientLogoURL
...
func (s *Store) EditorByID(ctx context.Context,userID,presentationID string,allowAll bool)(EditorInput,SavedDraft,error){
	...
	if err:=s.pool.QueryRow(ctx,`select p.id::text,p.title,p.created_by::text,coalesce(c.legal_name,''),coalesce(c.trade_name,''),coalesce(c.cnpj,'') from presentations p left join clients c on c.id=p.client_id where p.id=$1`,presentationID).Scan(&input.PresentationID,&input.Title,&owner,&input.ClientLegalName,&input.ClientTradeName,&input.ClientCNPJ);err!=nil{return EditorInput{},SavedDraft{},err}
	...
	if len(contentJSON)>0{
		...
		if json.Unmarshal(contentJSON,&content)==nil{
			...
			input.ClientLogoURL=content.Client.LogoURL
...
func (s *Store) PublicByToken(ctx context.Context,token string)(PublicPresentation,error){
	...
	err:=s.pool.QueryRow(ctx,`select p.id::text,v.id::text,v.version_number,v.public_token::text,p.title,v.content,v.content_hash from presentation_versions v join presentations p on p.id=v.presentation_id where v.public_token=$1 and v.published_at is not null and p.status='published'`,token).Scan(&result.PresentationID,&result.VersionID,&result.VersionNumber,&result.PublicToken,&currentTitle,&contentJSON,&result.ContentHash)
	...
	result.ClientLegalName=content.Client.LegalName;result.ClientTradeName=content.Client.TradeName;result.ClientLogoURL=content.Client.LogoURL
