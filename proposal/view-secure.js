...
function safeExternalUrl(value, allowedHosts = []) {
  ...
  try {
    ...
    const validHost = !allowedHosts.length || allowedHosts.some((host) => url.hostname === host || url.hostname.endsWith(`.${host}`));
    return validProtocol && validHost ? url.toString() : '';
  } catch {
    return '';
  }
}
...
function renderSocialLinks(salesperson) {
  const linkedin = safeExternalUrl(salesperson.linkedin, ['linkedin.com']);
  const instagram = safeExternalUrl(salesperson.instagram, ['instagram.com']);
...
function showGate(mode) {
  ...
  startButton.textContent = continuing ? 'CONTINUAR PROPOSTA' : 'VER PROPOSTA';
...
function bindViewerEvents() {
  startButton.addEventListener('click', startProposal);
  restartButton.addEventListener('click', restartProposal);
...
function removeAccessGate() {
  document.getElementById('proposalAccessGate')?.remove();
}

function showPasswordGate(invalidPassword = false) {
  loadingElement.hidden = true;
  document.body.classList.add('proposal-locked');
  document.getElementById('proposalGate').style.visibility = 'hidden';

  let gate = document.getElementById('proposalAccessGate');

  if (!gate) {
    gate = document.createElement('section');
    gate.className = 'proposal-access-gate';
    gate.id = 'proposalAccessGate';
    gate.innerHTML = `
      <form class="proposal-access-card" id="proposalAccessForm">
        <img src="../assets/logo-viagate-white.svg" alt="ViaGate" />
        <small>PROPOSTA COMERCIAL PROTEGIDA</small>
        <h1>Acesso à proposta</h1>
        <p>Informe a senha definida pelo responsável comercial para visualizar este material.</p>
        <input id="proposalAccessPassword" type="password" autocomplete="current-password" required autofocus placeholder="Senha" />
        <button type="submit">ACESSAR PROPOSTA</button>
        <p class="proposal-access-error" id="proposalAccessError"></p>
      </form>
    `;
    document.body.appendChild(gate);

    gate.querySelector('#proposalAccessForm')?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const button = event.currentTarget.querySelector('button');
      const password = gate.querySelector('#proposalAccessPassword')?.value ?? '';
      const errorElement = gate.querySelector('#proposalAccessError');

      button.disabled = true;
      button.textContent = 'VALIDANDO...';
      if (errorElement) errorElement.textContent = '';

      const authorized = await loadProposal(password, true);

      if (!authorized) {
        button.disabled = false;
        button.textContent = 'ACESSAR PROPOSTA';
        if (errorElement) errorElement.textContent = 'Senha inválida.';
        gate.querySelector('#proposalAccessPassword')?.focus();
      }
    });
  }
  ...
  const errorElement = gate.querySelector('#proposalAccessError');
  if (errorElement) {
    errorElement.textContent = invalidPassword ? 'Senha inválida.' : '';
  }
}

async function loadProposal(password = null, fromPasswordGate = false) {
  const { data, error } = await supabase.rpc('get_public_proposal_secure', {
    proposal_token: viewerState.token,
  ...
  if (data.requires_password && !data.authorized) {
    showPasswordGate(Boolean(password));
    return false;
  }

  removeAccessGate();
  document.getElementById('proposalGate').style.visibility = '';
  renderProposal(data);
  return true;
}
