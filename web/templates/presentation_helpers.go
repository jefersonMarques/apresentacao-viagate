...
func PresentationModuleSelected(input presentations.EditorInput, id string) bool {
	for _, value := range input.SelectedModules {
		if value == id {
			return true
		}
	}
...
func PresentationClientName(value presentations.PublicPresentation) string {
	if strings.TrimSpace(value.ClientTradeName) != "" {
		return value.ClientTradeName
	}
	if strings.TrimSpace(value.ClientLegalName) != "" {
		return value.ClientLegalName
	}
	return "sua operação"
}

func PresentationModulesCSV(values []string) string { return strings.Join(values, ",") }
