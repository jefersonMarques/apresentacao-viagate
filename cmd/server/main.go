...
func main() {
	...
	if err != nil {
		logger.Error("S3 unavailable", "error", err)
		os.Exit(1)
	}
	...
	if err := bootstrapAdmin(ctx, cfg, authStore, pool, logger); err != nil {
	...
	finalizationWorker := contracts.NewFinalizationWorker(pool, finalizer, logger)
	go finalizationWorker.Run(ctx)
	...
	app := httpapp.New(httpapp.Dependencies{
		...
		PipelineStore: pipelineStore, OnboardingStore: onboardingStore, ContractStore: contractStore, ActivationStore: activationStore, AuditStore: auditStore,
		ContractRenderer: renderer, ContractGenerator: generator, ContractFinalizer: finalizer, Storage: storageClient, Mailer: mailer, Registry: registry,
	})
	server := &http.Server{
...
func bootstrapAdmin(ctx context.Context, cfg config.Config, store *auth.Store, pool *pgxpool.Pool, logger *slog.Logger) error {
	...
	name := strings.TrimSpace(cfg.Bootstrap.AdminName)
	if name == "" {
		name = "Super Admin"
	}
	...
	created, err := store.BootstrapAdmin(ctx, emailAddress, name, hash, time.Now().Add(cfg.Session.InviteTTL))
	if err != nil {
		return err
	}
	if !created {
	...
	if err := notifications.Enqueue(ctx, pool, name, emailAddress, "Ativação do Super Admin ViaGate", htmlBody, "Ative o acesso em "+link); err != nil {
		return err
	}
	logger.Warn("bootstrap super admin invitation created", "email", emailAddress)
	return nil
}
